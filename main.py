...
class Matrixassistant:
    def __init__(self, root):
        self.root = root
        self.root.title("毕业基质自动识别工具beta")
        self.root.geometry("540x820") # 稍微加宽以容纳输入框
        ...
        # UI 上的停止提示
        tk.Label(root, text="( 扫描开启后，随时按键盘 'B' 键可强制停止 )",
                 font=("微软雅黑", 9), fg="#666666").pack()
        ...
        tk.Label(root, text="实时扫描日志:", font=("微软雅黑", 11)).pack(anchor="w", padx=10)
        ...
        tk.Label(root, text="已锁定基质列表:", font=("微软雅黑", 11, "bold")).pack(anchor="w", padx=10)
    ...
    def is_already_locked(self, sct, lock_pos, win_rect):
        try:
            ...
            return np.mean(gray) < 150
        except:
            return False

    def start_thread(self):
        if not all(self.data.values()):
            messagebox.showwarning("提示", "当前未进行初始化配置，请先进行配置")
            return
        self.log_area.delete('1.0', tk.END)
        self.lock_list_area.delete('1.0', tk.END)
        self.gui_log("[系统] 扫描启动，按键盘 'B' 键可紧急停止任务", "blue")
        self.running = True
    ...
    def add_to_lock_list(self, weapon_name, attrs, pos_str, star_level):
        name_tag = "red_text" if "6" in star_level else "gold_text"
        self.lock_list_area.insert(tk.END, weapon_name + " ", name_tag)
        self.lock_list_area.insert(tk.END, attrs.replace(",", "，") + " ", "green_text")
        self.lock_list_area.insert(tk.END, "坐标" + pos_str + "\n", "black_text")
    ...
    def on_press(self, key):
        try:
            if hasattr(key, 'char') and key.char == 'b':
                if self.running:
                    self.gui_log("[停止] 用户按下 B 键，任务已中止", "red")
                    self.running = False
        except:
            pass
    ...
    def clear_config(self):
        self.data = {"roi": None, "grid": None, "lock": None}
    ...
    def get_click(self, prompt, callback):
        ...
        ov.attributes("-alpha", 0.3, "-fullscreen", True, "-topmost", True)
    ...
    def set_roi(self):
        ...
        with mss.mss() as sct:
            ...
            roi = cv2.selectROI("SELECT_ROI (Enter确认)", cv2.cvtColor(img, cv2.COLOR_BGRA2BGR), False)
    ...
    def set_lock(self):
        self.get_click("点击锁定图标位置", lambda rx, ry: [self.data.update({"lock": (rx, ry)}), self.save_config()])

    def clean_text(self, raw_text):
        if not raw_text: return ""
        # 核心改进：逻辑映射修正 力量
        txt = re.sub(r'[^\u4e00-\u9fa5]', '', str(raw_text))
        if "辐" in txt or "撮" in txt:
            txt = txt.replace("辐", "力量").replace("撮", "力量")
        if "政" in txt:
            txt = txt.replace("政", "攻")
        return txt
    ...
    def fuzzy_match(self, target, text):
        ...
        n = len(target_c)
        threshold = 0.45 if n <= 2 else 0.72
        for i in range(len(text) - n + 1):
            if difflib.SequenceMatcher(None, target_c, text[i:i + n]).ratio() >= threshold:
                return True
        return False
    ...
    def is_gold(self, cell_bgr):
        try:
            ...
            strip = cell_bgr[int(h * 0.88):int(h * 0.98), :]
            hsv = cv2.cvtColor(strip, cv2.COLOR_BGR2HSV)
            mask = cv2.inRange(hsv, np.array([18, 140, 140]), np.array([32, 255, 255]))
            return (np.sum(mask > 0) / mask.size) > 0.10
        except:
            return False
        ...
        with mss.mss() as sct:
            ...
            while self.running:
                ...
                try:
                    ...
                    current_speed = 0.5 # 如果输入非法则默认为0.5
                ...
                for c in range(9):
                    ...
                    bgr = cv2.cvtColor(snap, cv2.COLOR_BGRA2BGR)

                    if self.is_gold(bgr):
                        self.gui_log(f"-----检查: {current_row + 1}-{c + 1} -----", "black")
                        pydirectinput.click(abs_x, abs_y)
                        time.sleep(current_speed) # 使用动态速度
                        ...
                        inverted = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                                         cv2.THRESH_BINARY_INV, 15, 10)

                        h, w = inverted.shape
                        slice_h, full_clean_txt, clean_list = h // 3, "", []
                        for i in range(3):
                            ...
                            if res:
                                ...
                                if txt: clean_list.append(txt); full_clean_txt += txt
                        ...
                        if full_clean_txt:
                            ...
                            for weapon in self.weapon_list:
                                c1, c2, c3 = weapon.get('毕业词条1', ''), weapon.get('毕业词条2', ''), weapon.get(
                                    '毕业词条3', '')
                                if all(self.fuzzy_match(cx, full_clean_txt) for cx in [c1, c2, c3]):
                ...
                current_row += 1
                if current_row >= 5:
                    self.gui_log(f"[系统] 第 {current_row} 行完成，翻页...", "black")
                    pydirectinput.moveTo(int(cur_win[0] + grid["rx"] + 4 * grid["rdx"]),
                                         int(cur_win[1] + grid["ry"] + 2 * grid["rdy"]))
                    pyautogui.scroll(int(grid["rdy"] * (0.7 if current_row == 5 else 1.0) * -0.8))
                    time.sleep(1.0)
